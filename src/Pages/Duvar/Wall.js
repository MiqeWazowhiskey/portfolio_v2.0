...
import {  collection, doc, QuerySnapshot, setDoc } from 'firebase/firestore/lite'
...
function Wall({user}) {
    ...
    const res=async(script) => {await setDoc(doc(db,'/scripts'),script).then(()=>{setScript("")})}
    return (
    <div className='flex flex-col  justify-center items-center gap-y-8 md:justify-center md:items-center md:gap-y-4'>
        ...
        <div className='flex justify-center h-16 border p-2 gap-x-2'>
            ...
            <button className='px-5  rounded-lg border bg-gray-100' onClick={res(script)} >Submit</button>
        </div>
        <div className='flex'>
